...
def run_game(session_number, combined_log_file):
    ...
    board = chess.Board()
    move_history = []  # Initialize move history here
    pieces = load_images()
                ...
                if gemini_san_move:
                    try:
                        if gemini_san_move in [board.san(move) for move in board.legal_moves]:
                            ...
                            return False, move_history  # Return move_history on failure
                    except Exception as e:
    ...
    return True, move_history  # Return move_history after the game

    
def save_move_history(session_number, move_history, filename="v14_200_all_sessions_moves.txt"):
    """Appends the move history of a session in a single line to the specified file."""
    with open(filename, 'a') as f:
        moves_str = ', '.join(move_history)  # Join moves in a single line
        f.write(f"{moves_str}\n")

def main():
    combined_log_file = "v14_200_combined_game_sessions.txt"  # One file for all 10 games
    move_history_file = "v14_200_all_sessions_moves.txt"  # File to store move history for each session
    
    with open(combined_log_file, 'w') as f:
        f.write("SessionID$PromptNo$P/R$PromptOrResponse\n")

    with open(move_history_file, 'w') as f:
        f.write("Session Move History\n")  # Header for move history file

    for game_number in range(1, 201):  # Run 100 games
        game_result, move_history = run_game(game_number, combined_log_file)
        
        save_move_history(game_number, move_history, move_history_file)  # Save moves after game
        
        if not game_result:
            print(f"Game {game_number} stopped due to invalid move by Gemini. Moving to the next game.")
            continue
    print("Session finished.")
    pygame.quit()  # Close Pygame properly after all sessions
